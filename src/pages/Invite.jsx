...
function Invite() {
  ...
  const handleJoinLink = async () => {
    if (!userId || !linkInput) return alert("Please paste a link.");
    const token = linkInput.split("/").pop();

    const q = query(collection(db, "sharedDashboards"), where("token", "==", token));
    const snapshot = await getDocs(q);
    if (snapshot.empty) return alert("Invalid or expired link.");
    ...
    if (alreadyJoined) return alert("Already joined this dashboard.");
  ...
  // Leave joined dashboard
//   const handleLeave = async (docId, token) => {
